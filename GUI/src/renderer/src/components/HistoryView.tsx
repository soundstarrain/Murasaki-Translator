...
export function HistoryView({ lang }: { lang: Language }) {
  ...
  const handleExportLog = (record: TranslationRecord) => {
    ...
    if (record.triggers.length > 0) {
      lines.push(
        e.triggersTitle.replace("{count}", record.triggers.length.toString()),
      );
                      ...
                      <div className="grid grid-cols-7 gap-3 text-sm">
                        <div>
                          ...
                          <p className="font-medium">
                            {record.completedBlocks}/{record.totalBlocks}
                          </p>
                        </div>
                        <div>
                          <p className="text-muted-foreground text-xs">
                          ...
                          <p
                            ...
                            title={`${lang === "zh" ? "源" : "Src"}: ${record.sourceLines || "-"} / ${lang === "zh" ? "译" : "Dst"}: ${record.totalLines || "-"}`}
                          >
                            {record.sourceLines || "-"}{" "}
                        ...
                        <div>
                          ...
                          <p
                            className="font-medium truncate"
                            title={`${lang === "zh" ? "源" : "Src"}: ${record.sourceChars?.toLocaleString() || "-"} / ${lang === "zh" ? "译" : "Dst"}: ${record.totalChars?.toLocaleString() || "-"}`}
                          >
                            {record.sourceChars?.toLocaleString() || "-"}{" "}
                            <span className="text-muted-foreground">/</span>{" "}
                            {record.totalChars?.toLocaleString() || "-"}
                          </p>
                        </div>
                        <div>
                          <p className="text-muted-foreground text-xs">
                          ...
                          <p className="font-medium">
                            {record.avgSpeed
                              ? `${record.avgSpeed} ${t.dashboard.charPerSec}`
                              : "-"}
                          </p>
                        </div>
                        <div>
                      ...
                      {record.triggers.length > 0 && (
                        <div>
                          <p className="text-xs font-medium text-muted-foreground mb-2">
                            {t.advancedView.validationRules}
                          </p>
