...
import { FileUp, ArrowRight, X, AlertTriangle, CheckCircle2, FileJson, Sparkles, RefreshCw } from 'lucide-react'
...
export function GlossaryConverter({ onClose, onSuccess, lang = 'zh', initialFile }: GlossaryConverterProps) {
    ...
    const parseTxtContent = (content: string) => {
        ...
        lines.forEach((line, idx) => {
            ...
            if (k && v) {
                ...
                const val = v.trim()
                if (glossary[key]) {
                    console.debug(`[Converter] Overwriting entry at line ${idx + 1}: ${key}`)
                    overwrites++
                }
                glossary[key] = val
            }
            ...
            if (Array.isArray(data)) {
                data.forEach((entry, idx) => {
                    ...
                    const getVal = (keys: string[]) => {
                        const foundKey = Object.keys(entry).find(k => keys.includes(k.toLowerCase()))
                    ...
                    if (src && dst) {
                        const s = String(src)
                        if (glossary[s]) {
                            console.debug(`[Converter] Overwriting entry at index ${idx}: ${s}`)
                            overwrites++
                        }
    ...
    const finalizeParse = (glossary: Record<string, string>, overwrites: number) => {
        const count = Object.keys(glossary).length
        if (count > 0) {
            setStatus('success')
            if (overwrites > 0) {
                console.info(`[Converter] Parsed ${count} entries with ${overwrites} overwrites.`)
            }
        } else {
            setStatus('error')
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            window.api?.showNotification('Murasaki Translator', t.glossaryConverter.saveSuccess)
            onSuccess()
            onClose()
        } catch (e) {
            showAlert({ title: "保存失败", description: (e as Error).message, variant: 'destructive' })
        }
    }
            ...
            <Card className="w-full max-w-4xl max-h-[85vh] flex flex-col shadow-[0_0_50px_-12px_rgba(138,43,226,0.3)] border-primary/20 bg-background/95 backdrop-blur-xl animate-in zoom-in-95 duration-200">
                <CardHeader className="flex flex-row items-center justify-between border-b px-6 py-4 bg-secondary/10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <CardTitle className="text-lg font-bold tracking-tight">{t.glossaryConverter.title}</CardTitle>
                            <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mt-0.5 opacity-70">Legacy Format ➔ Murasaki JSON</p>
                        </div>
                    </div>
                ...
                <CardContent className="flex-1 overflow-hidden flex flex-col p-8 gap-8">
                    ...
                    <div className="flex items-center justify-center gap-4 max-w-md mx-auto w-full">
                        <div className={`flex items-center gap-2 ${status === 'idle' ? 'text-primary' : 'text-muted-foreground'}`}>
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border-2 ${status === 'idle' ? 'border-primary bg-primary/10' : 'border-muted bg-transparent'}`}>1</div>
                            <span className="text-xs font-bold">{t.glossaryConverter.stepRead}</span>
                        </div>
                        <div className="h-px flex-1 bg-border" />
                        <div className={`flex items-center gap-2 ${status === 'parsing' ? 'text-primary' : 'text-muted-foreground'}`}>
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border-2 ${status === 'parsing' ? 'border-primary bg-primary/10' : 'border-muted bg-transparent'}`}>2</div>
                            <span className="text-xs font-bold">{t.glossaryConverter.stepParse}</span>
                        </div>
                        <div className="h-px flex-1 bg-border" />
                        <div className={`flex items-center gap-2 ${status === 'success' ? 'text-primary' : 'text-muted-foreground'}`}>
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border-2 ${status === 'success' ? 'border-primary bg-primary/10' : 'border-muted bg-transparent'}`}>3</div>
                            <span className="text-xs font-bold">{t.glossaryConverter.stepExport}</span>
                        </div>
                    </div>
                            ...
                            <div className="relative group cursor-pointer w-full max-w-lg">
                                ...
                                <div className="border-2 border-dashed border-primary/20 rounded-3xl p-12 flex flex-col items-center justify-center gap-4 bg-primary/5 group-hover:bg-primary/10 group-hover:border-primary/40 transition-all duration-300">
                                    ...
                                    <div className="text-center">
                                        <p className="text-sm font-bold text-foreground">{t.glossaryConverter.dropzoneTitle}</p>
                                        <p className="text-xs text-muted-foreground mt-1">{t.glossaryConverter.dropzoneDesc}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {status === 'parsing' && (
                        <div className="flex-1 flex flex-col items-center justify-center gap-4">
                            ...
                            <p className="text-sm font-medium animate-pulse">{t.glossaryConverter.parsingStatus}</p>
                        </div>
                    )}
                                ...
                                <div className="lg:col-span-3 border border-border/60 rounded-2xl overflow-hidden flex flex-col bg-secondary/20 backdrop-blur-sm relative">
                                    <div className="flex items-center justify-between bg-card border-b px-4 py-2">
                                        <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/70">
                                            <Sparkles className="w-3 h-3 text-primary" /> {t.glossaryConverter.previewTitle}
                                        </div>
                                        <div className="flex bg-secondary rounded-lg p-0.5 border">
                                            ...
                                            >{t.glossaryView.viewModeTable}</button>
                                            <button
                                            ...
                                            >{t.glossaryView.viewModeRaw}</button>
                                        </div>
                                    </div>
                                    ...
                                    {viewMode === 'table' ? (
                                        <div className="flex-1 overflow-y-auto divide-y divide-border/40 scrollbar-thin">
                                            ...
                                            {Object.entries(parsedEntries).slice(0, 200).map(([k, v], i) => (
                                                <div key={i} className="grid grid-cols-2 px-4 py-2.5 hover:bg-primary/5 transition-colors text-xs items-center border-border/20">
                                ...
                                <div className="lg:col-span-2 flex flex-col gap-6">
                                    ...
                                    <div className="p-5 bg-card border border-border/80 rounded-2xl shadow-sm space-y-5">
                                        <div className="flex items-center gap-3 pb-4 border-b">
                                            ...
                                            <div>
                                                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none mb-1.5">{t.glossaryConverter.parseSuccess}</p>
                                                <p className="text-lg font-black tracking-tight">{t.glossaryConverter.itemsCount.replace('{count}', Object.keys(parsedEntries).length.toString())}</p>
                                            </div>
                                        </div>

                                        <div className="space-y-4">
                                            <div className="space-y-2">
                                                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1">{t.glossaryConverter.saveFileName}</label>
                                        ...
                                        <Button className="w-full gap-2 h-12 text-sm font-bold rounded-xl shadow-lg shadow-primary/20 group mt-4 relative overflow-hidden" onClick={handleSave}>
                    ...
                    {status === 'error' && (
                        <div className="flex-1 flex flex-col items-center justify-center gap-6 animate-in zoom-in-95">
                            ...
                            <div className="text-center max-w-sm">
                                <p className="font-bold text-red-600 mb-1">{t.glossaryConverter.parseError}</p>
                                <p className="text-xs text-muted-foreground leading-relaxed">{errorMsg}</p>
                            </div>
                            <Button variant="outline" size="sm" onClick={() => setStatus('idle')} className="rounded-xl px-6">
